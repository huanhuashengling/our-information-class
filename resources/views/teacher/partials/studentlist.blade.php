@foreach ($students as $student)
    ...
    <div class="col-md-2 col-sm-4 col-xs-6" style="padding-left: 5px; padding-right: 5px;">
        <div class="alert {{$postCss}}" style="padding: 5px;">
            ...
            <div><h3 style="margin-top: 10px;">{{ @$student->order_num }} {{ $py->getFirstchar($student->username) }} <small>{{ $student->username }}<small></small><span class="text-right"> {{$ratestr}}{{$marksNum}}{{$commentStr}}</span></small></h3>  </div>
        </div>
    </div>
@endforeach